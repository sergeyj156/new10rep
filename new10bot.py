...
API_TOKEN = '7152142719:AAGrCtZnEe5TSoxesaBfV805EmoUXKnyz2g'
...
async def get_points(callback_query: types.CallbackQuery):
    ...
    points_message = await bot.send_message(callback_query.from_user.id, "Выберите один из вариантов:",
...
async def send_post(callback_query: types.CallbackQuery):
    ...
    with open('foto_l/7.jpg', 'rb') as photo:  # Замените 'path_to_image.jpg' на путь к вашему изображению
        post_message = await bot.send_photo(callback_query.from_user.id, photo,
                                            caption="Чтобы получить баллы за пост, изучи правила в карточке 😉\n\n"
                                                    "Если твой пост соответствует требованиям, отправляй на него ссылку вместе с хэштегом #100балльныйрепетитор и картинку.",
                                            reply_markup=InlineKeyboardMarkup().add(
                                                InlineKeyboardButton("Вернуться назад", callback_data='main_menu')))
...
async def generate_link(callback_query: types.CallbackQuery):
    ...
    link_message = await bot.send_message(callback_query.from_user.id, f"Вот твоя ссылка для приглашения в каналы: https://t.me/testoviy56_bot?start={referral_code}\n\n"
...
async def show_achievements(callback_query: types.CallbackQuery):
    ...
    # Путь к изображению
    image_path = 'foto_l/9.jpg'  # Замените на реальный путь к изображению
    ...
    with open(image_path, 'rb') as photo:
        achievements_message = await bot.send_photo(
            ...
            caption="Гонка за призами этого месяца в самом разгаре!\n\n"
                    "Чекай карточку, чтобы узнать, какие призы тебя ждут в июне 🔥",
            reply_markup=InlineKeyboardMarkup().add(
                InlineKeyboardButton("Вернуться назад", callback_data='main_menu')
            )
        )
...
async def show_help(callback_query: types.CallbackQuery):
    ...
    help_message = await bot.send_message(callback_query.from_user.id, "Пожалуйста, опиши свою проблему, чтобы менеджер быстрее нашёл решение",
                                          reply_markup=InlineKeyboardMarkup().add(InlineKeyboardButton("Вернуться назад", callback_data='main_menu')))
    user_message_ids[callback_query.from_user.id] = [help_message.message_id]

    # Ожидание сообщения от пользователя
    @dp.message_handler()
    async def handle_help_message(message: types.Message):
        admin_id = [679030634, 927878071]  # Замените на ID вашего администратора
...
async def start_test(callback_query: types.CallbackQuery):
    ...
    age_markup.add(
        InlineKeyboardButton('17-18', callback_data='age_17_18'),
